...
def debug(port):
    ...
    logger.info("Failed as debugging not implemented yet.")
...
class BaseAcceleratorConfig:
    ...
    mixed_precision: PrecisionType = PrecisionType.BF16 # changed from PrecisionType.NO
    gradient_accumulation_steps: int = 1
    log_with: Optional[LoggerType] = LoggerType.WANDB
    debug: DebugConfig = DebugConfig()
    seed: int = 42
    resume_from_checkpoint: bool = True
    max_steps: int = 70000 #150000
    num_epochs: int = 10
    ...
    save_at_manual: List[int] = field(default_factory=lambda: [14000,40000])
...
class BaseAccelerator(abc.ABC):
    ...
    def load_state_if_needed(self):
        ...
        try:
            ...
            logger.error(f"Error loading checkpoint {ckpt_path}: {e}. Trying again with strict=False")
            self.accelerator.load_state(ckpt_path, load_module_strict=False)
        logger.info("Checkpoint loaded")
        # logger.info("Note: strict loading is disabled, may be problem e.g., for learnable positional encoding")
    ...
    def should_save(self):
        return self.sync_gradients and self.global_step > 0 and self.cfg.save_steps > 0 and ((self.global_step % self.cfg.save_steps == 0) or (self.global_step in self.cfg.save_at_manual))
    ...
    def get_all_ckpts(self):
        ...
        all_ckpts = [
            ckpt for ckpt in glob(os.path.join(self.cfg.output_dir, f"checkpoint-*"))
            if not any(ckpt.endswith(f"{num}") for num in self.cfg.save_at_manual) # don't include manual checkpoints, for count or deletion
        ]
        return all_ckpts
    ...
    def cleanup_checkpoints(self):
        ...
        logger.info(f"Found {len(all_ckpts)} checkpoints in {self.cfg.output_dir}. (Note this does not include the {len(self.cfg.save_at_manual)} manual checkpoints.)")
    ...
    #         ema_output_dir = os.path.join(output_dir, "unet_ema")
    ...
    #         model.ema_unet.load_state_dict(load_model.state_dict())
    #         model.ema_unet.to(self.device)
    #         del load_model
